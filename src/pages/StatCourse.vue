<template>
    <div>
        <el-row style="padding: 12px 48px;padding-top: 48px;line-height: 48px;">
            <el-input placeholder="课程/学员名称" v-model="queryWord" class="input-with-select" size="small"
        ...
        <el-row style="padding: 12px 48px;">
            <el-table :data="courseStudyList" border style="width: 100%">
                <el-table-column prop="courseName" label="课程名称" width="500" show-overflow-tooltip></el-table-column>
                <el-table-column prop="userName" label="学员" width="150"></el-table-column>
                <el-table-column prop="totalStudyTime" label="累计学习时长" width="200"></el-table-column>
                <el-table-column prop="classHour" label="课时" width="100"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            getCourseStudyList: function () {
                var that = this;
                axios.get(Config.URL_BASE + '/stat/course/list', {
                    params: {
                    ...
                    that.courseStudyList = response.data.data.items;
                    for (var i = 0; i < that.courseStudyList.length; i++) {
                        that.courseStudyList[i].totalStudyTime = that.timeConvert(that.courseStudyList[i].totalStudyTime);
                    }
